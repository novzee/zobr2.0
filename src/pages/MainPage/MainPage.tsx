      ...
      >
        <Form className={s.Form}>
          ...
          <div className={s.Items}>
            {items.map((el) => (
              <div className={s.Item} key={el.filmId || el.kinopoiskId}>
                <Link to={`/movie/${el.filmId || el.kinopoiskId}`}><div className={s.Item_img} style={{backgroundImage: `url(${el.posterUrlPreview})`}}><div className={s.back}>
                ...
                <h5 className={s.name}>{el.nameRu}</h5>
              </div>
            ))}
          </div>
        </div>
